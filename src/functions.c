...
#include <math.h>
#define SQUARE(x) (x)*(x)
#define CUBE(x) (x)*(x)*(x) //macro's for ² and ³
void draw_F(double x, double y);
void draw_O(double x, double y); //protofunctions for letters used inside of other letters
...
void move(double theta_1, double theta_2) { //Moving the servo's given the angles (assuming that this will always be necessary)
    return;
}
...
void lin_bez(double start_x, double start_y, double end_x, double end_y) { //Function for Linear bezier curve
...
//the arguments x and y are the discrete coordinates of the tiles on the 6x5 wordle grid, with (0,0) being the tile in the TOP left corner!
